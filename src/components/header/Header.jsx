import { useState } from "react";
import { useCart } from "../../context/CartContext";
import logo from "../../assets/Black White Simple Fashion Logo.png";
import styles from "./Header.module.css";
import MegaMenu from "../megaMenu/MegaMenu";

function Header({onOpenSearch}) {
  const { setIsCartOpen, getTotalItems } = useCart();
  const cartCount = getTotalItems();
  const [openMenu, setOpenMenu] = useState(null);

  const handleMenuToggle = (category) => {
    if (openMenu === category) {
      setOpenMenu(null);
    } else {
      setOpenMenu(category);
    }
  };

  return (
    <>
      <header className={styles.header}>
        <div>
          <img src={logo} alt="logo" />
        </div>
        {/* header's info */}
        <div className={styles.buttonsContainer}>
          <button
            onClick={() => handleMenuToggle("Women")}
            className={openMenu === "Women" ? styles.active : ""}
          >
            <p>Women</p>
            <p>{openMenu === "Women" ? "−" : "+"}</p>
          </button>
          <button
            onClick={() => handleMenuToggle("Man")}
            className={openMenu === "Man" ? styles.active : ""}
          >
            <p>Man</p>
            <p>{openMenu === "Man" ? "−" : "+"}</p>
          </button>
          <button>About Us</button>
        </div>
      <div className={styles.lastComponent}>
        <button style={{display:'flex',alignItems :'center',justifyContent:'space-between'}}>
          Change Language
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={1.5}
            stroke="currentColor"
            className={styles.headerSvg}
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="m19.5 8.25-7.5 7.5-7.5-7.5"
            />
          </svg>
        </button>
        <div className={styles.svgContainer}>
          <button onClick={onOpenSearch} aria-label="Search">
            {/* search button */}
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className={styles.headerSvg}
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z"
              />
            </svg>
          </button>
          <button aria-label="Wishlist">
            {/* wishlist button */}
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className={styles.headerSvg}
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z"
              />
            </svg>
          </button>
          <button onClick={() => setIsCartOpen(true)} aria-label="Shopping Cart" className={styles.cartButton}>
            {/* cart button */}
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className={styles.headerSvg}
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z"
              />
            </svg>
            {cartCount > 0 && (
              <span className={styles.cartBadge}>{cartCount}</span>
            )}
          </button>
        </div>
      </div>
      </header>
      <MegaMenu
        category={openMenu}
        isOpen={openMenu !== null}
        onClose={() => setOpenMenu(null)}
      />
    </>
  );
}
export default Header;
